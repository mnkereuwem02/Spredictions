...
price = st.slider("Product Price", float(df["Price_x"].min()), float(df["Price_x"].max()), float(df["Price_x"].mean()))
competitor_price = st.slider("Competitor Price", float(df["Price_y"].min()), float(df["Price_y"].max()), float(df["Price_y"].mean()))
...
order_year = st.selectbox("Order Year", sorted(df["Order Year"].unique()))
...
if st.button("Predict Results"):
    ...
    ml_input = np.array([[price, competitor_price, price_ratio, segment_encoded, ship_mode_encoded, state_encoded, city_encoded, region_encoded,category_encoded,sub_category_encoded, order_year]])
...
st.subheader("Summary Table: Demand Elasticity Metrics by Customer Segment")

# Group the DataFrame by Customer Segment and calculate average values
...
summary_table.columns = ["Customer Segment", "Avg Log(Price)", "Avg Log(Quantity)", "Avg Price Ratio"]
